...
export const WORKSPACE_ACTION_FLAG_DESCRIPTIONS: Readonly<Record<string, string>> = {
  ...
  '--refresh-graph': 'Bypass the compatible persisted graph and rebuild from live sources.',
  ...
  '--kind': 'Limit graph entity search to one canonical entity kind.',
  ...
  '--plan': 'Resolve lifecycle commands without executing project code.',
  '--runtime': 'Limit lifecycle planning or execution to one detected runtime family.',
...
export const WORKSPACE_ACTION_CONTRACTS = {
  ...
  run: {
    ...
    examples: [
      'workspai workspace run test --scope project:api --plan --runtime ruby --json',
