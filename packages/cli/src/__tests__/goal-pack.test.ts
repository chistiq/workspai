...
describe('goal pack workspace adapter', () => {
  ...
  it('accepts coverage evidence regenerated by its own CLI verification receipt', async () => {
    const { workspacePath, projectPath } = await fixture();
    const coveragePath = path.join(projectPath, 'coverage', 'coverage-summary.json');
    const coverage = (covered: number) => ({
      total: {
        lines: { total: 100, covered, skipped: 0, pct: covered },
        statements: { total: 100, covered, skipped: 0, pct: covered },
      },
    });
    await fsExtra.outputJson(coveragePath, coverage(50));
    await fsExtra.outputFile(
      path.join(projectPath, 'write-coverage.cjs'),
      `require('fs').writeFileSync('coverage/coverage-summary.json', ${JSON.stringify(
        JSON.stringify(coverage(85))
      )});\n`
    );
    const packageJson = await fsExtra.readJson(path.join(projectPath, 'package.json'));
    packageJson.scripts = {
      ...packageJson.scripts,
      'test:coverage': 'node write-coverage.cjs',
      build: 'node -e "process.exit(0)"',
    };
    await fsExtra.writeJson(path.join(projectPath, 'package.json'), packageJson, { spaces: 2 });
    await writeWorkspaceModel(
      await buildWorkspaceModel({ workspacePath, includeAbsolutePaths: true }),
      workspacePath
    );

    const planned = await planGoalPack({
      startPath: projectPath,
      intent: 'Keep test coverage at or above 80 percent',
      scope: 'project:api',
      runtime: 'node',
    });
    const originalEvidenceHash = planned.goalPack.preflight.measurement.existingEvidence[0]?.sha256;
    await prepareGoalVerification({ workspacePath, goalId: planned.goalPack.id });

    const verified = await verifyGoalLifecycle({
      workspacePath,
      goalId: planned.goalPack.id,
      run: true,
    });
    const currentEvidenceHash = createHash('sha256')
      .update(await fsExtra.readFile(coveragePath))
      .digest('hex');

    expect(currentEvidenceHash).not.toBe(originalEvidenceHash);
    expect(verified.goal.verificationReceipt).toBeDefined();
    await expect(
      inspectGoalLifecycle({ workspacePath, goalId: planned.goalPack.id })
