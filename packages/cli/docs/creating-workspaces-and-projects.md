...
| `agent.google-adk.python` | Python | Pin from `version-baselines.v1.json` (`google-adk`) | Isolated `agents/<instance>/` with pip-editable `pyproject.toml`, credentialless `unittest`, provider profiles `gemini-api` / `vertex-ai`. Preview; Create is fail-closed until release admission. |
| `agent.google-adk.typescript` | Node.js | Pin from `version-baselines.v1.json` (`@google/adk`) | Isolated `agents/<instance>/` with `package.json`, credentialless `node:test`, provider profiles `gemini-api` / `vertex-ai`. Preview; Create is fail-closed until release admission. Independent from the Python runtime. |
