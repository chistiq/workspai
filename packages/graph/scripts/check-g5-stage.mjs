...
const planPath = 'packages/graph/governance/g5-stage-plan.v1.json';
const closurePath = 'packages/graph/governance/g5-stage-closure.v1.json';
const remoteCheckpoint = 'g5-cross-platform-admission';
...
function digestFiles(files) {
  ...
  for (const relative of [...files].sort()) {
    ...
    if (!fs.existsSync(file) || !fs.lstatSync(file).isFile()) {
      throw new Error(`Missing G5 evidence: ${relative}`);
    }
...
if (
  ...
  failures.push('G5 plan identity or fail-closed boundaries drifted');
}
if (
  closure.stage !== 'G5' ||
  closure.status !== 'local-passed-remote-pending-awaiting-approval' ||
  closure.advancesAdmissionGate !== false ||
  closure.nextStage !== 'G6' ||
  closure.nextStageAuthorized !== false
) {
  failures.push('G5 closure does not record local pass with remote admission pending');
}
if (manifest.private !== true || manifest.publishable === true) {
  failures.push('G5 candidate must remain private and non-publishable');
}
...
for (const checkpoint of checkpoints) {
  ...
  if (checkpoint.status !== 'implemented-local-candidate') {
    failures.push(`G5 checkpoint ${checkpoint.id} is not implemented-local-candidate`);
  }
  for (const evidence of checkpoint.evidence ?? []) {
    const file = repositoryFile(evidence);
    if (!fs.existsSync(file) || !fs.lstatSync(file).isFile()) {
      failures.push(`Missing G5 checkpoint evidence: ${evidence}`);
    }
  }
}
if (!checkpoints.some((checkpoint) => checkpoint.id === remoteCheckpoint)) {
  failures.push(`G5 plan must retain ${remoteCheckpoint}`);
}
if (/(?:[A-Za-z]:\\|\/home\/|\/Users\/)/u.test(JSON.stringify({ plan, closure }))) {
  failures.push('G5 governance contains a machine-local path');
}

const report = {
  schemaVersion: 'workspai-graph-g5-stage-audit.v1',
