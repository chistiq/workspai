...
for (const name of fs
  ...
  if (
    report.schemaVersion !== 'workspai-graph-repository-preview-admission-audit.v1' ||
    report.stage !== 'G4' ||
    report.status !== 'passed-platform'
  ) {
    failures.push(`${String(runnerOs)}: repository preview admission failed`);
  }
...
const candidate = {
  schemaVersion: 'workspai-graph-repository-preview-matrix-admission.v1',
