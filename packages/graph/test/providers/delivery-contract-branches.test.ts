...
): GraphProviderDetectionResult['status'] {
  return (
    provider.detect({
      availableInputs,
      scopeKind: 'project',
      networkAllowed: false,
    }) as GraphProviderDetectionResult
  ).status;
}
...
describe('delivery contract provider branches', () => {
  ...
  it('extracts GraphQL schema and executable definition variants', async () => {
    ...
    expect(detectStatus(provider, ['README.md'])).toBe('not-applicable');
    expect(detectStatus(provider, ['schema.gql'])).toBe('applicable');
  ...
  it('admits architecture decisions and documentation surfaces while keeping unreadable files partial', async () => {
    ...
    expect(detectStatus(decisions, ['README.md'])).toBe('not-applicable');
    ...
    expect(detectStatus(documents, ['src/index.ts'])).toBe('not-applicable');
